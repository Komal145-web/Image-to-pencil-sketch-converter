...
def sketch_image(image_path):
    ...
    # here we are converting the image to grayscale
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # here we inverted the grayscale image
    inverted_gray_image = 255 - gray_image

    # here we applied GaussianBlur to the inverted image
    blurred_image = cv2.GaussianBlur(inverted_gray_image, (21, 21), 0)

    # then again we inverted the blurred image
...
def upload_image():
    ...
    if file:
        ...
        # here we are processing the image and then we are saving the output
